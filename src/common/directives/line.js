angular.module('myGraph', ['D3Service'])
.directive('myGraph', ['$timeout', '$window', 'D3Service', function($timeout, $window, D3Service) {
    return {
        restrict: 'E',
        scope: {
            data: '='
        ...
        link: function(scope, element, attrs) {
            ...
            var render = function(data, canvas) {
                ...
                vis = d3.select(element[0]).data(data).append('svg').attr('width', w).attr('height', h + padt + padb).attr('class', 'viz').append('svg:g').attr('transform', "translate(" + padl + "," + padt + ")");
                ...
                if (add_gradient) {
                    ...
                    gradient
                        .append("svg:stop")
                        .attr("offset", 100 * y(warn_at) / h + '%')
                        .attr("stop-color", "#396")
                        .attr("stop-opacity", 1);
                    gradient
                        .append("svg:stop")
                        .attr("offset", 100 * y(transition) / h + '%')
                        .attr("stop-color", "#F29D50")
                        .attr("stop-opacity", 1);
                    gradient
                        .append("svg:stop")
                        .attr("offset", 100 * y(bad_at) / h + '%')
                        .attr("stop-color", "#c00")
                        .attr("stop-opacity", 1);
                }
                vis.append("g")
