...
.controller( 'DashboardTrafficCtrl', function DashboardTrafficController( $scope, transferStatistics ) {
    ...
    /*traffic.codeStatsByDate = transferStatistics.aggregations.history.buckets.map(function(history){
        return getCodesFromBuckets(history.statuses.buckets);
    });*/
    
    traffic.codeStatsByDate = {
        '500': transferStatistics.aggregations.history.buckets.map(function(history) {
            var suma = 0;
            var total = history.statuses.buckets.reduce(function(previousValue, currentValue, index, arra){
                var key = parseInt(currentValue.key, 10);
                var sum = (key >= 500) ? currentValue.doc_count + previousValue: previousValue;
                //console.log("key: ", key, "\tcnt: ", currentValue.doc_count, "\tsum: ", sum, "\tprev: ", previousValue);
                return sum;
            }, 0);
    ...
    /*transferStatistics.aggregations.history.buckets.map(function(history){
        return getCodesFromBuckets(history.statuses.buckets);
    });*/

    console.log("traffic.codeStatsByDate: ", traffic.codeStatsByDate);
